...
from typing import Any, Dict, List, Optional, Type  # Added Type
...
# --- Configuration ---
LLAMA_CPP_INTERNAL_MODEL_PATH_FOR_REWOO = "/home/kal/code/models/Qwen3-8B.Q4_K_M.gguf"

# --- Mock Tools (Unchanged) ---
@tool
async def get_stock_ticker(company_name: str, context: Dict[str, Any]) -> Dict[str, Optional[str]]:
...
@tool
async def get_stock_price(ticker_symbol: str, context: Dict[str, Any]) -> Dict[str, Optional[float]]:
...
@tool
async def get_latest_news(company_name_or_ticker: str, context: Dict[str, Any], num_headlines: int = 1) -> Dict[str, List[str]]:
    news_db = {
        "microsoft": ["Microsoft launches new AI chip series.", "Microsoft quarterly earnings exceed expectations."],
...
async def run_complex_rewoo_test():
    ...
    if not model_path.exists() or LLAMA_CPP_INTERNAL_MODEL_PATH_FOR_REWOO == "/path/to/your/model.gguf":
    ...
    app_config = MiddlewareConfig(
        ...
        command_processor_configurations={
            "rewoo_command_processor_v1": { "max_plan_retries": 0 }
        }
    )
    ...
    try:
        ...
        mock_solver_llm_response_content = LLMChatResponse(message={"role": "assistant", "content": expected_final_answer_str})
        ...
        async def mock_genie_llm_parse_output_for_rewoo_test(response: Any, schema: Type[PydanticBaseModel], **kwargs):
            ...
            if schema and hasattr(schema, "__name__") and "DynamicReWOOPlan" in schema.__name__:
                ...
                print("MOCK PARSE_OUTPUT: Intercepted call for ReWOO Plan. Returning mock plan instance based on DynamicReWOOPlan schema.")
                return schema(**mock_plan_data.model_dump())
            return await original_genie_llm_parse_output(response=response, schema=schema, **kwargs)

        async def mock_genie_llm_chat_for_rewoo_test(messages: List[ChatMessage], provider_id: Optional[str] = None, **kwargs):
            nonlocal solver_chat_call_count
            prompt_content_for_solver_check = messages[-1]["content"]
            if isinstance(prompt_content_for_solver_check, str) and \
               "Original Goal:" in prompt_content_for_solver_check and \
               "The following evidence was gathered" in prompt_content_for_solver_check:
                solver_chat_call_count += 1
                print("MOCK CHAT: Intercepted call for ReWOO Solver. Returning mock solver response.")
                return mock_solver_llm_response_content
            return await original_genie_llm_chat(messages=messages, provider_id=provider_id, **kwargs)
        ...
        if command_result.get("llm_thought_process"):
            try:
                ...
                assert thought_data["evidence"][2]["result"] == {"headlines": ["Microsoft launches new AI chip series."]}
        ...
        genie.execute_tool.assert_any_call("get_stock_ticker", company_name="Microsoft")
        genie.execute_tool.assert_any_call("get_stock_price", ticker_symbol="MSFT")
        genie.execute_tool.assert_any_call("get_latest_news", company_name_or_ticker="Microsoft", num_headlines=1)
