...
class TestBootstrapMechanism:
    async def test_genie_create_runs_bootstrap_plugin(
        ...
        """
        # --- FIX START ---
        # The instance returned by the patched class should be an AsyncMock for async classes.
        # This ensures all its awaitable methods are correctly mocked.
        mock_rag_manager_instance = AsyncMock(spec=RAGManager)
        # Also provide a valid dictionary return value, as the code in RAGInterface calls .get() on the result.
        mock_rag_manager_instance.index_data_source.return_value = {
            "status": "mocked_success",
            "added_count": 1,
        }
        MockRAGManager.return_value = mock_rag_manager_instance
        # --- FIX END ---
        ...
        mock_plugin_manager_for_bootstrap.get_plugin_instance.side_effect = (
            get_instance_side_effect
        )

        test_config = MiddlewareConfig(
            features=FeatureSettings(
                rag_embedder="sentence_transformer", rag_vector_store="faiss"
            )
        )

        # 2. Action
        genie = await Genie.create(
            config=test_config, plugin_manager=mock_plugin_manager_for_bootstrap
        )

        # 3. Assertions
        mock_plugin_manager_for_bootstrap.get_all_plugin_instances_by_type.assert_awaited_once_with(
            BootstrapPlugin
        )
        ...
        mock_rag_manager_instance.index_data_source.assert_awaited_once_with(
            loader_id=ANY,
    ...
    async def test_genie_create_succeeds_with_no_bootstrap_plugins(
        ...
        try:
            await Genie.create(
                ...
                plugin_manager=mock_plugin_manager_for_bootstrap,
            )
        except Exception as e:
            pytest.fail(
                f"Genie.create() raised an unexpected exception with no bootstrap plugins: {e}"
            )
    ...
    async def test_genie_create_fails_if_bootstrap_plugin_raises_error(
        self,
        MockRAGManager,
        mock_plugin_manager_for_bootstrap: MagicMock,
