...
class MockToolForSimpleProcessor(ToolPlugin):
    def __init__(self, identifier: str, input_schema: Dict[str, Any]):
        self.identifier = identifier
        self.plugin_id = identifier
        self._input_schema = input_schema
        self._name = f"MockTool_{identifier}"

    async def get_metadata(self) -> Dict[str, Any]:
        return {
            "identifier": self.identifier,
            "name": self._name,
            "description_llm": f"Description for {self._name}",
            "input_schema": self._input_schema,
            "output_schema": {"type": "object"},
        }

    async def execute(self, params: Dict[str, Any], key_provider: Any, context: Optional[Dict[str, Any]] = None) -> Any:
        return {"result": "mock_tool_executed", "params_received": params}
...
async def test_process_command_tool_not_found_in_manager(processor: SimpleKeywordToolSelectorProcessorPlugin, mock_genie_facade: MagicMock):
    ...
    response = await processor.process_command("calculate 1+1")
    assert response.get("error") == "Internal error: Tool 'calculator' not found after keyword match."
...
async def test_process_command_success_with_params(
    mock_builtin_input: MagicMock,
    ...
    adder_schema = {
        ...
        "properties": {
            ...
            "op_name": {"type": "string", "description": "Operation", "default": "sum"},
        },
        "required": ["num1", "num2"],
    }
...
@pytest.mark.asyncio
@patch("builtins.input")
async def test_process_command_param_coercion(
    mock_builtin_input: MagicMock,
    processor: SimpleKeywordToolSelectorProcessorPlugin,
    mock_genie_facade: MagicMock
):
    ...
    type_schema = {
        ...
        "properties": {
            "p_int": {"type": "integer"},
            "p_float": {"type": "number"},
            "p_bool_true": {"type": "boolean"},
            "p_bool_false": {"type": "boolean"},
            "p_str": {"type": "string"},
        },
        "required": ["p_int", "p_float", "p_bool_true", "p_bool_false", "p_str"],
    }
    ...
    assert response.get("extracted_params") == {
        "p_int": 10, "p_float": 3.14, "p_bool_true": True, "p_bool_false": False, "p_str": "hello"
    }

@pytest.mark.asyncio
@patch("builtins.input")
async def test_process_command_required_param_not_provided(
    mock_builtin_input: MagicMock,
    processor: SimpleKeywordToolSelectorProcessorPlugin,
    mock_genie_facade: MagicMock
):
...
@pytest.mark.asyncio
@patch("builtins.input")
async def test_process_command_optional_param_skipped(
    mock_builtin_input: MagicMock,
    processor: SimpleKeywordToolSelectorProcessorPlugin,
    mock_genie_facade: MagicMock
):
...
@pytest.mark.asyncio
@patch("builtins.input")
async def test_process_command_optional_param_provided(
    mock_builtin_input: MagicMock,
    processor: SimpleKeywordToolSelectorProcessorPlugin,
    mock_genie_facade: MagicMock
):
...
@pytest.mark.asyncio
async def test_empty_command_string(
    processor: SimpleKeywordToolSelectorProcessorPlugin,
    mock_genie_facade: MagicMock
):
...
@pytest.mark.asyncio
@patch("builtins.input")
async def test_tool_with_no_params(
    mock_builtin_input: MagicMock,
    processor: SimpleKeywordToolSelectorProcessorPlugin,
    mock_genie_facade: MagicMock
):
...
@pytest.mark.asyncio
@patch("builtins.input")
async def test_param_with_enum_in_prompt(
    mock_builtin_input: MagicMock,
    processor: SimpleKeywordToolSelectorProcessorPlugin,
    mock_genie_facade: MagicMock
):
    ...
    enum_schema = {
        "type": "object",
        "properties": {
            "choice": {"type": "string", "description": "Pick one.", "enum": ["A", "B", "C"]}
        },
        "required": ["choice"],
    }
...
@pytest.mark.asyncio
async def test_process_command_with_conversation_history_ignored(
    processor: SimpleKeywordToolSelectorProcessorPlugin,
    mock_genie_facade: MagicMock
):
...
@pytest.mark.asyncio
async def test_teardown_method(processor: SimpleKeywordToolSelectorProcessorPlugin):
    try:
        await processor.teardown()
    except Exception as e:
        pytest.fail(f"SimpleKeywordToolSelectorProcessorPlugin.teardown() raised an unexpected exception: {e}")