...
@pytest.fixture()
def mock_log_adapter_instance() -> MagicMock:
...
def tracing_manager(
    ...
    mock_plugin_manager_for_obs_mgr.get_plugin_instance.side_effect = (
...
async def test_initialize_tracers_success(
    ...
    mock_plugin_manager_for_obs_mgr.get_plugin_instance.side_effect = (
        get_instance_side_effect_console
    )
    ...
    assert (
        tracing_manager_for_console_test._active_tracers[0]
        is mock_console_tracer_plugin_instance
    )
...
async def test_initialize_tracers_plugin_not_found(
    ...
    assert (
        "InteractionTracerPlugin 'non_existent_tracer' not found or failed to load."
        in caplog.text
    )
...
async def test_initialize_tracers_plugin_wrong_type(
    ...
    mock_plugin_manager_for_obs_mgr.get_plugin_instance.return_value = (
        wrong_type_plugin_instance
    )
    ...
    assert len(manager._active_tracers) == 0
    assert (
        "Plugin 'wrong_type_tracer' loaded but is not a valid InteractionTracerPlugin."
        in caplog.text
    )
...
async def test_initialize_tracers_load_error(
    ...
    mock_plugin_manager_for_obs_mgr.get_plugin_instance.side_effect = RuntimeError(
        "Load failed"
    )
    manager = InteractionTracingManager(
        plugin_manager=mock_plugin_manager_for_obs_mgr,
    ...
    assert len(manager._active_tracers) == 0
    assert (
        "Error loading InteractionTracerPlugin 'error_tracer': Load failed"
        in caplog.text
    )
...
async def test_trace_event_no_active_tracers(
    ...
    await manager.trace_event(
        event_name="test", data={}, component="comp", correlation_id="cid"
    )
...
async def test_trace_event_tracer_error(
    ...
    caplog.set_level(logging.ERROR, logger=MANAGER_LOGGER_NAME)
    await tracing_manager._initialize_tracers()
    mock_tracer_plugin.record_trace.side_effect = RuntimeError(
        "Trace recording failed"
    )

    await tracing_manager.trace_event("error.event", {}, "ErrorComp", "err-cid")
    assert (
        f"Error recording trace with tracer '{mock_tracer_plugin.plugin_id}': Trace recording failed"
        in caplog.text
    )
...
async def test_teardown_calls_tracer_teardown(
    ...
    assert (
        tracing_manager._log_adapter_instance is None
    )  # Check log adapter is cleared
...
async def test_teardown_tracer_teardown_error(
    ...
    await tracing_manager.teardown()
    assert (
        f"Error tearing down tracer '{mock_tracer_plugin.plugin_id}': Teardown error"
        in caplog.text
    )
    assert len(tracing_manager._active_tracers) == 0