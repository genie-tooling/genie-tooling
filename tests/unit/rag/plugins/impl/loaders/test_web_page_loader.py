...
async def web_loader() -> AsyncGenerator[WebPageLoader, None]:
    ...
    try:
        await loader.setup() # Call setup to initialize client
        yield loader
    finally:
...
async def test_load_non_html_content_type(web_loader: AsyncGenerator[WebPageLoader, None], dummy_request: httpx.Request, caplog: pytest.LogCaptureFixture):
    ...
    assert any(f"Content type for http://example.com/data.json is '''application/json''', not HTML. Using raw content." in rec.message for rec in caplog.records)
