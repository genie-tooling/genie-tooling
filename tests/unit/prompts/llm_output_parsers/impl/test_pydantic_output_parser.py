...
# --- REFACTOR: Remove all tests for the now non-existent private `_extract_json_block` method ---
...
async def test_parse_malformed_json_in_code_block_pydantic(pydantic_parser: PydanticOutputParserPlugin, caplog: pytest.LogCaptureFixture):
    ...
    # --- REFACTOR: Check the final warning from the parser itself ---
    with caplog.at_level(logging.WARNING, logger=PARSER_LOGGER_NAME):
        with pytest.raises(ValueError, match="No parsable JSON block found"):
