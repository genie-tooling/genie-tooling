...
from genie_tooling.lookup.providers.impl.embedding_similarity import logger as es_logger
...
def es_lookup_provider(mock_plugin_manager_for_es_lookup: PluginManager) -> EmbeddingSimilarityLookupProvider:
    """
    Provides an instance of EmbeddingSimilarityLookupProvider.
    Note: This is now a synchronous fixture. Tests will call `await provider.setup()`
    """
...
async def test_es_setup_no_plugin_manager(
    ...
    # For simple logs not involving exceptions, caplog can still work if logger name is correct
    caplog.set_level(logging.ERROR, logger="genie_tooling.lookup.providers.impl.embedding_similarity")
...
async def test_es_index_tools_no_embedder(
    ...
    await es_lookup_provider.index_tools([{"identifier": "t1", "lookup_text_representation": "desc"}])
...
async def test_es_index_tools_embed_failure(
    ...
    with patch.object(es_logger, "error") as mock_log_error:
        # Corrected test data to use "identifier"
        await es_lookup_provider.index_tools([{"identifier": "t1", "lookup_text_representation": "desc"}])
    ...
    expected_msg_content = f"{es_lookup_provider.plugin_id}: Error during tool text embedding for index: Simulated embedder failure"
    mock_log_error.assert_called_once_with(expected_msg_content)
...
async def test_es_find_tools_index_not_built(
    ...
    caplog.set_level(logging.WARNING, logger="genie_tooling.lookup.providers.impl.embedding_similarity")
...
async def test_es_find_tools_query_embed_failure(
    ...
    assert results == []
    expected_msg_content = f"{es_lookup_provider.plugin_id}: Error embedding lookup query: Simulated embedder failure"
    mock_log_error.assert_called_once_with(expected_msg_content)
...
async def test_es_find_tools_empty_query(
    ...
    caplog.set_level(logging.DEBUG, logger="genie_tooling.lookup.providers.impl.embedding_similarity")
    ...
    assert results == []
    assert f"{es_lookup_provider.plugin_id}: Empty query provided. Returning no results." in caplog.text

    caplog.clear()
    results_whitespace = await es_lookup_provider.find_tools("   ")
...
async def test_es_index_tools_mismatched_embedding_dimensions(
    ...
    # The first error expected is due to np.array conversion
    expected_np_conversion_error_fragment = "Failed to convert embeddings to NumPy array"
    # The mismatch error (length check) should *not* be called if the np.array fails first as it does.
    # If the np.array conversion *passed* but lengths were different (which is hard to achieve with current np.array),
    # then the mismatch log would occur. Here, the ValueError from np.array from inhomogeneous shapes is the first hit.

    found_np_conversion_log = False
    for call_args in mock_log_error.call_args_list:
        logged_message = call_args.args[0] # The first arg to logger.error
        # The second arg, if present and True, would be exc_info=True
        has_exc_info = call_args.kwargs.get("exc_info", False)

        if expected_np_conversion_error_fragment in logged_message and has_exc_info:
            found_np_conversion_log = True
            break

    assert found_np_conversion_log, f"Log for np.array conversion error not found or exc_info not True. Calls: {mock_log_error.call_args_list}"
