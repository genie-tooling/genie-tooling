...
class TestToolLookupService:
    async def test_initialization(self, tool_lookup_service: ToolLookupService):
        ...
        assert tool_lookup_service._default_indexing_formatter_id == DEFAULT_INDEXING_FORMATTER_ID
        assert not tool_lookup_service._is_indexed_map

    async def test_find_tools_triggers_full_reindex_on_first_call(self, tool_lookup_service: ToolLookupService, mock_tool_lookup_provider: MagicMock):
        with patch.object(tool_lookup_service, "reindex_all_tools", new_callable=AsyncMock) as mock_reindex:
            await tool_lookup_service.find_tools("test query")
            mock_reindex.assert_awaited_once()

        with patch.object(tool_lookup_service, "reindex_all_tools", new_callable=AsyncMock) as mock_reindex_2:
    ...
    async def test_reindex_all_tools_success(self, tool_lookup_service: ToolLookupService, mock_tool_manager: MagicMock, mock_tool_lookup_provider: MagicMock):
        ...
        mock_tool2.get_metadata = AsyncMock(return_value={"identifier": "tool2", "name": "Tool Two"})
        ...
        assert call_args["tools_data"][0]["lookup_text_representation"] == "Formatted: Tool One"

    async def test_reindex_all_tools_provider_load_fails(self, tool_lookup_service: ToolLookupService, mock_plugin_manager: MagicMock, caplog):
    ...
    async def test_find_tools_empty_query(self, tool_lookup_service: ToolLookupService, mock_tool_lookup_provider: MagicMock):
        results = await tool_lookup_service.find_tools("   ")
        assert results == []
        mock_tool_lookup_provider.find_tools.assert_not_called()

    async def test_find_tools_provider_search_fails(self, tool_lookup_service: ToolLookupService, mock_tool_lookup_provider: MagicMock, caplog):
        caplog.set_level(logging.ERROR, logger=SERVICE_LOGGER_NAME)
        tool_lookup_service._is_indexed_map[DEFAULT_LOOKUP_PROVIDER_ID] = True
        ...
        assert results == []
        assert f"Error finding tools with provider '{DEFAULT_LOOKUP_PROVIDER_ID}': Search failed" in caplog.text

    async def test_invalidate_all_indices(self, tool_lookup_service: ToolLookupService):
