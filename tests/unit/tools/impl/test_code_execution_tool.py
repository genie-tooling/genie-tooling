...
def mock_plugin_manager_for_code_exec(mocker) -> PluginManager:
    pm = mocker.MagicMock(spec=PluginManager); pm.list_discovered_plugin_classes = MagicMock(return_value={}); pm.get_plugin_instance = AsyncMock()
    return pm
@pytest.fixture
def mock_key_provider_for_code_exec(mocker) -> KeyProvider:
    kp = mocker.AsyncMock(spec=KeyProvider); return kp
...
async def test_generic_code_exec_tool_init_and_get_metadata_no_executors(mock_plugin_manager_for_code_exec: PluginManager):
    ...
    assert metadata["identifier"] == "generic_code_execution_tool"; assert "No code executors currently available." in metadata["description_human"]
    assert "Supported languages: None" in metadata["description_llm"]; assert metadata["input_schema"]["properties"]["language"].get("enum") is None
...
async def test_generic_code_exec_tool_get_metadata_with_executors(mock_plugin_manager_for_code_exec: PluginManager):
    py_executor_res = CodeExecutionResult("py_out", "", None, None, 10.0); js_executor_res = CodeExecutionResult("js_out", "", None, None, 12.0)
    ...
    async def get_instance_side_effect(plugin_id_req: str, config: Optional[Dict[str, Any]] = None):
        if plugin_id_req == "py_exec_plugin": await mock_py_executor.setup(config); return mock_py_executor
        if plugin_id_req == "js_exec_plugin": await mock_js_executor.setup(config); return mock_js_executor
        return None
    ...
    metadata = await tool.get_metadata()
    assert "python_sandbox_v1" in metadata["description_human"]; assert "nodejs_sandbox_v1" in metadata["description_human"]
    assert "python" in metadata["input_schema"]["properties"]["language"]["enum"]; assert "javascript" in metadata["input_schema"]["properties"]["language"]["enum"]
    assert "python_sandbox_v1" in metadata["input_schema"]["properties"]["executor_id"]["enum"]; assert "nodejs_sandbox_v1" in metadata["input_schema"]["properties"]["executor_id"]["enum"]
...
async def test_execute_python_code_auto_select_executor(mock_plugin_manager_for_code_exec: PluginManager, mock_key_provider_for_code_exec: KeyProvider):
    expected_stdout = "Hello Python!"; py_executor_res = CodeExecutionResult(expected_stdout, "", None, None, 25.5)
    ...
    tool = GenericCodeExecutionTool(plugin_manager=mock_plugin_manager_for_code_exec); params = {"language": "python", "code_to_run": "logger.debug('Hello Python!')"}
    result_dict = await tool.execute(params, mock_key_provider_for_code_exec)
    assert result_dict["stdout"] == expected_stdout; assert result_dict["error"] is None; assert result_dict["execution_time_ms"] == 25.5
...
async def test_execute_code_with_specific_executor_id(mock_plugin_manager_for_code_exec: PluginManager, mock_key_provider_for_code_exec: KeyProvider):
    expected_output = "Specific exec works"; exec_res = CodeExecutionResult(expected_output, "", None, None, 30.0)
    ...
    async def get_instance_side_effect(plugin_id_req: str, config=None):
        ...
        return None
    mock_plugin_manager_for_code_exec.get_plugin_instance.side_effect = get_instance_side_effect
    tool = GenericCodeExecutionTool(plugin_manager=mock_plugin_manager_for_code_exec); params = {"language": "python", "code_to_run": "...", "executor_id": "executor_one"}
    result_dict = await tool.execute(params, mock_key_provider_for_code_exec)
    assert result_dict["stdout"] == expected_output; assert result_dict["error"] is None
...
async def test_execute_unsupported_language(mock_plugin_manager_for_code_exec: PluginManager, mock_key_provider_for_code_exec: KeyProvider):
    ...
    mock_plugin_manager_for_code_exec.list_discovered_plugin_classes.return_value = {"py_exec": type(mock_py_executor)}
    mock_plugin_manager_for_code_exec.get_plugin_instance.return_value = mock_py_executor
    tool = GenericCodeExecutionTool(plugin_manager=mock_plugin_manager_for_code_exec); params = {"language": "ruby", "code_to_run": "puts 'hello'"}
    result_dict = await tool.execute(params, mock_key_provider_for_code_exec)
    assert result_dict["error"] == "No suitable executor."; assert "No available executor found that supports language 'ruby'" in result_dict["stderr"]
...
async def test_execute_requested_executor_not_found(mock_plugin_manager_for_code_exec: PluginManager, mock_key_provider_for_code_exec: KeyProvider):
    ...
    tool = GenericCodeExecutionTool(plugin_manager=mock_plugin_manager_for_code_exec); params = {"language": "python", "code_to_run": "...", "executor_id": "non_existent_executor"}
    result_dict = await tool.execute(params, mock_key_provider_for_code_exec)
    assert result_dict["error"] == "Executor not found."; assert "Requested executor 'non_existent_executor' not found or not available." in result_dict["stderr"]
...
async def test_execute_requested_executor_does_not_support_language(mock_plugin_manager_for_code_exec: PluginManager, mock_key_provider_for_code_exec: KeyProvider):
    ...
    tool = GenericCodeExecutionTool(plugin_manager=mock_plugin_manager_for_code_exec); params = {"language": "python", "code_to_run": "...", "executor_id": "js_sandbox"}
    result_dict = await tool.execute(params, mock_key_provider_for_code_exec)
    assert result_dict["error"] == "Executor language mismatch."; assert "Requested executor 'js_sandbox' does not support language 'python'" in result_dict["stderr"]
...
async def test_execute_executor_itself_raises_error(mock_plugin_manager_for_code_exec: PluginManager, mock_key_provider_for_code_exec: KeyProvider):
    mock_erroring_executor = AsyncMock(spec=CodeExecutor); mock_erroring_executor.plugin_id = "error_exec"; mock_erroring_executor.executor_id = "error_sandbox"
    mock_erroring_executor.description = "Errors"; mock_erroring_executor.supported_languages = ["python"]; mock_erroring_executor.setup = AsyncMock(); mock_erroring_executor.teardown = AsyncMock()
    ...
    tool = GenericCodeExecutionTool(plugin_manager=mock_plugin_manager_for_code_exec); params = {"language": "python", "code_to_run": "..."}
    result_dict = await tool.execute(params, mock_key_provider_for_code_exec)
