...
class MockCalcKeyProvider(KeyProvider): # No plugin_id needed if not registered
    async def get_key(self, key_name: str) -> Optional[str]: return None
    async def setup(self,c=None): pass
    async def teardown(self): pass

@pytest.fixture
async def calculator_tool() -> CalculatorTool: # Async fixture returns instance
    tool = CalculatorTool()
    # await tool.setup() # CalculatorTool has no specific setup
    return tool

@pytest.fixture
async def mock_calc_key_provider() -> MockCalcKeyProvider: # Async fixture
    return MockCalcKeyProvider()

@pytest.mark.asyncio
async def test_calculator_tool_get_metadata(calculator_tool: CalculatorTool):
    actual_tool = await calculator_tool # <<<< AWAIT FIXTURE
...
@pytest.mark.parametrize("num1, num2, operation, expected_result, expected_error", [
    ...
    ("abc", 5, "add", None, "Invalid number input: could not convert string to float: 'abc'. Both num1 and num2 must be valid numbers."),
])
@pytest.mark.asyncio
async def test_calculator_tool_execute(
    ...
    actual_tool = await calculator_tool # <<<< AWAIT FIXTURE
    actual_kp = await mock_calc_key_provider # <<<< AWAIT FIXTURE
    ...
    assert result_dict["error_message"] == expected_error
...
async def test_calculator_tool_execute_missing_param(calculator_tool: CalculatorTool, mock_calc_key_provider: MockCalcKeyProvider):
    ...
    assert "Missing required parameter: 'num2'" in result_dict["error_message"] # type: ignore
