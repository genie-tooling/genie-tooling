...
# anext was added in Python 3.10. Provide a fallback for compatibility.
try:
...
@pytest.fixture()
async def fs_tool_with_sandbox(
    tmp_path: Path,
) -> AsyncGenerator[SandboxedFileSystemTool, None]:
...
async def test_fs_get_metadata(fs_tool_with_sandbox: AsyncGenerator[SandboxedFileSystemTool, None]):
    ...
    assert metadata["input_schema"]["properties"]["operation"]["enum"] == [
        "read_file",
        "write_file",
        "list_directory",
    ]
...
async def test_fs_setup_success_creates_path(tmp_path: Path):
    ...
    assert (
        sandbox_dir_to_create.exists()
    ), "Sandbox directory should have been created by tool's setup"
    assert sandbox_dir_to_create.is_dir()


@pytest.mark.asyncio()
async def test_fs_setup_fail_path_is_file(
    ...
    assert (
        f"Configured 'sandbox_base_path' ({file_path.resolve()}) exists but is not a directory."
        in caplog.text
    )
...
@pytest.mark.asyncio()
async def test_fs_resolve_secure_path_success(
    fs_tool_with_sandbox: AsyncGenerator[SandboxedFileSystemTool, None],
):
    tool = await anext(fs_tool_with_sandbox)
...
@pytest.mark.asyncio()
async def test_fs_resolve_secure_path_fail_traversal(
    fs_tool_with_sandbox: AsyncGenerator[SandboxedFileSystemTool, None],
):
    tool = await anext(fs_tool_with_sandbox)
    assert tool._sandbox_root is not None, "Sandbox root should be initialized by the fixture"
...
@pytest.mark.asyncio()
async def test_fs_resolve_secure_path_fail_absolute_path(
    fs_tool_with_sandbox: AsyncGenerator[SandboxedFileSystemTool, None],
):
    tool = await anext(fs_tool_with_sandbox)
    assert tool._sandbox_root is not None, "Sandbox root should be initialized by the fixture"
...
@pytest.mark.asyncio()
async def test_fs_execute_sandbox_not_initialized(
    mock_fs_key_provider: MockFSKeyProvider,
):
    tool_no_setup = SandboxedFileSystemTool()
    result = await tool_no_setup.execute(
        {"operation": "read_file", "path": "test.txt"}, mock_fs_key_provider, context={}
    )
...
@pytest.mark.asyncio()
async def test_fs_execute_read_file_success(
    fs_tool_with_sandbox: AsyncGenerator[SandboxedFileSystemTool, None],
    mock_fs_key_provider: MockFSKeyProvider,
):
    tool = await anext(fs_tool_with_sandbox)
    ...
    result = await tool.execute(
        {"operation": "read_file", "path": "readable.txt"},
        mock_fs_key_provider,
        context={},
    )
...
@pytest.mark.asyncio()
async def test_fs_execute_read_file_not_found(
    fs_tool_with_sandbox: AsyncGenerator[SandboxedFileSystemTool, None],
    mock_fs_key_provider: MockFSKeyProvider,
):
    tool = await anext(fs_tool_with_sandbox)
    assert tool._sandbox_root is not None, "Sandbox root should be initialized by the fixture"
    result = await tool.execute(
        {"operation": "read_file", "path": "non_existent_file.txt"},
        mock_fs_key_provider,
        context={},
    )
    assert result["success"] is False
...
@pytest.mark.asyncio()
async def test_fs_execute_write_file_success_create_and_read_back(
    fs_tool_with_sandbox: AsyncGenerator[SandboxedFileSystemTool, None],
    mock_fs_key_provider: MockFSKeyProvider,
):
    tool = await anext(fs_tool_with_sandbox)
    ...
    write_result = await tool.execute(
        {"operation": "write_file", "path": file_path_str, "content": content},
        mock_fs_key_provider,
        context={},
    )
    ...
    read_result = await tool.execute(
        {"operation": "read_file", "path": file_path_str},
        mock_fs_key_provider,
        context={},
    )
...
@pytest.mark.asyncio()
async def test_fs_execute_write_file_fail_no_overwrite(
    fs_tool_with_sandbox: AsyncGenerator[SandboxedFileSystemTool, None],
    mock_fs_key_provider: MockFSKeyProvider,
):
    tool = await anext(fs_tool_with_sandbox)
    ...
    result = await tool.execute(
        {"operation": "write_file", "path": "exists.txt", "content": "new", "overwrite": False},
        mock_fs_key_provider,
        context={},
    )
    assert result["success"] is False
    assert "File exists and overwrite is false" in result["message"]  # type: ignore
    async with aiofiles.open(existing_file, "r") as f:
        assert await f.read() == "original"


@pytest.mark.asyncio()
async def test_fs_execute_write_file_creates_parent_dirs(
    fs_tool_with_sandbox: AsyncGenerator[SandboxedFileSystemTool, None],
    mock_fs_key_provider: MockFSKeyProvider,
):
    tool = await anext(fs_tool_with_sandbox)
    ...
    write_result = await tool.execute(
        {"operation": "write_file", "path": deep_path_str, "content": content},
        mock_fs_key_provider,
        context={},
    )
    assert write_result["success"] is True
...
@pytest.mark.asyncio()
async def test_fs_execute_list_directory_success(
    fs_tool_with_sandbox: AsyncGenerator[SandboxedFileSystemTool, None],
    mock_fs_key_provider: MockFSKeyProvider,
):
    tool = await anext(fs_tool_with_sandbox)
    ...
    result = await tool.execute(
        {"operation": "list_directory", "path": "."}, mock_fs_key_provider, context={}
    )
    assert result["success"] is True
    assert sorted(result["file_list"]) == sorted(  # type: ignore
        ["file1.txt", "file2.log", "sub"]
    )

    result_sub = await tool.execute(
        {"operation": "list_directory", "path": "sub"},
        mock_fs_key_provider,
        context={},
    )
...
@pytest.mark.asyncio()
async def test_fs_execute_unknown_operation(
    fs_tool_with_sandbox: AsyncGenerator[SandboxedFileSystemTool, None],
    mock_fs_key_provider: MockFSKeyProvider,
):
    tool = await anext(fs_tool_with_sandbox)
    assert tool._sandbox_root is not None, "Sandbox root should be initialized by the fixture"
    result = await tool.execute(
        {"operation": "delete_everything", "path": "."},
        mock_fs_key_provider,
        context={},
    )
    assert result["success"] is False
