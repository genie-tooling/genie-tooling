...
class _ConcreteRetrievedChunk(RetrievedChunk):
    def __init__(self, id: TypingOptional[str], content: str, score: float, metadata: TypingOptional[Dict[str, TypingAny]] = None, rank: TypingOptional[int] = None):
        self.id = id
...
async def test_llm_interface_generate_success(llm_interface: LLMInterface, mock_llm_provider_manager: MagicMock):
    ...
    mock_provider.generate = AsyncMock(return_value=LLMCompletionResponse(text="Generated text", finish_reason="stop", usage=dummy_usage, raw_response={}))
...
async def test_llm_interface_chat_success(llm_interface: LLMInterface, mock_llm_provider_manager: MagicMock):
    ...
    mock_provider.chat = AsyncMock(return_value=LLMChatResponse(message=ChatMessage(role="assistant", content="Chat response"), finish_reason="stop", usage=dummy_usage, raw_response={}))
    mock_llm_provider_manager.get_llm_provider = AsyncMock(return_value=mock_provider)
...
@pytest.mark.asyncio
async def test_llm_interface_generate_provider_not_found_error(llm_interface: LLMInterface, mock_llm_provider_manager: MagicMock):
...
@pytest.mark.asyncio
async def test_llm_interface_input_guardrail_block_generate(llm_interface: LLMInterface, mock_guardrail_manager_for_llm: MagicMock):
...
async def test_llm_interface_output_guardrail_block_generate(llm_interface: LLMInterface, mock_llm_provider_manager: MagicMock, mock_guardrail_manager_for_llm: MagicMock):
    ...
    mock_provider.generate = AsyncMock(return_value=LLMCompletionResponse(text="Risky output", finish_reason="stop", usage=dummy_usage, raw_response={}))
    mock_llm_provider_manager.get_llm_provider = AsyncMock(return_value=mock_provider)
    mock_guardrail_manager_for_llm.check_output_guardrails = AsyncMock(return_value=GuardrailViolation(action="block", reason="Blocked by output guardrail"))
...
@pytest.mark.asyncio
async def test_rag_interface_index_web_page_missing_vector_store_id(rag_interface: RAGInterface, mock_middleware_config_for_rag: MagicMock):
...
@pytest.mark.asyncio
async def test_observability_interface_trace_event(observability_interface: ObservabilityInterface, mock_tracing_manager: MagicMock):
...
async def test_prompt_interface_list_templates(prompt_interface: PromptInterface, mock_prompt_manager: MagicMock):
    mock_prompt_manager.list_available_templates.return_value = [PromptIdentifier(name="p1", version="v1", description="d1")]
...
@pytest.mark.asyncio
async def test_conversation_interface_load_state(conversation_interface: ConversationInterface, mock_conversation_manager: MagicMock):
...
@pytest.mark.asyncio
async def test_conversation_interface_save_state(conversation_interface: ConversationInterface, mock_conversation_manager: MagicMock):
...
@pytest.mark.asyncio
async def test_conversation_interface_add_message(conversation_interface: ConversationInterface, mock_conversation_manager: MagicMock):
...
@pytest.mark.asyncio
async def test_conversation_interface_delete_state(conversation_interface: ConversationInterface, mock_conversation_manager: MagicMock):
