# genie-tooling/src/genie_tooling/tools/manager.py
...
class ToolManager:
    def __init__(
        ...
        self._tool_initial_configs: Dict[str, Dict[str, Any]] = (
            {}
        )  # Stores the initial tool_configurations
    ...
    async def _trace(
        ...
        if self._tracing_manager:
            final_event_name = (
                event_name if event_name.startswith("log.") else f"log.{level}"
            )
    ...
    async def initialize_tools(
        self, tool_configurations: Optional[Dict[str, Any]] = None
    ) -> None:
        self._tool_initial_configs = (
            tool_configurations or {}
        ...
        await self._trace(
            "log.debug",
            {
                "message": f"Initializing tools based on tool_configurations. Number of configured tools: {len(self._tool_initial_configs)}"
            },
        )

        for plugin_id_or_alias, tool_specific_config in self._tool_initial_configs.items():
            plugin_class = self._plugin_manager.list_discovered_plugin_classes().get(
                plugin_id_or_alias
            )
            ...
            if not plugin_class:
                await self._trace(
                    "log.debug",
                    {
                        "message": f"Tool ID/alias '{plugin_id_or_alias}' not found as a discovered plugin class. It may be a function-based tool to be registered later."
                    },
                )
                continue
            ...
            try:
                instance_any = await self._plugin_manager.get_plugin_instance(
                    plugin_id_or_alias, config=final_tool_config
                )
                ...
                if not instance_any or not isinstance(instance_any, Tool):
                    if instance_any:
                        await self._trace(
                            "log.debug",
                            {
                                "message": f"Plugin '{plugin_id_or_alias}' (class {plugin_class.__name__ if inspect.isclass(plugin_class) else plugin_class}) instantiated but is not a Tool."
                            },
                        )
                    else:
                        await self._trace(
                            "log.warning",
                            {
                                "message": f"Plugin '{plugin_id_or_alias}' (class {plugin_class.__name__ if inspect.isclass(plugin_class) else plugin_class}) did not yield a valid instance or failed setup."
                            },
                        )
                    continue
                ...
                if instance.identifier in self._tools:
                    await self._trace(
                        "log.warning",
                        {
                            "message": f"Duplicate tool identifier '{instance.identifier}' encountered from plugin ID/alias '{plugin_id_or_alias}'. Source: '{self._plugin_manager.get_plugin_source(plugin_id_or_alias)}'. Overwriting previous tool with same identifier."
                        },
                    )
                ...
                await self._trace(
                    "log.debug",
                    {
                        "message": f"Initialized tool: '{instance.identifier}' from plugin ID/alias '{plugin_id_or_alias}' (Source: {self._plugin_manager.get_plugin_source(plugin_id_or_alias)})"
                    },
                )
                ...
                await self._trace(
                    "log.error",
                    {
                        "message": f"Error initializing tool plugin from ID/alias '{plugin_id_or_alias}' (class {plugin_class}): {e}",
                        "exc_info": True,
                    },
                    level="error",
                )
        ...
        await self._trace(
            "log.info",
            {
                "message": f"ToolManager initialized. Loaded {len(self._tools)} explicitly configured class-based tools."
            },
        )

    async def register_decorated_tools(
        self, functions: List[Callable], auto_enable: bool
    ):
        """
        ...
        for func_item in functions:
            ...
            if not (
                ...
                and callable(original_func_to_call)
            ):
                await self._trace(
                    "log.warning",
                    {
                        "message": f"Function '{getattr(func_item, '__name__', str(func_item))}' not @tool decorated. Skipping."
                    },
                )
                continue
            ...
            if tool_id in self._tools:
                ...
                await self._trace(
                    "log.debug",
                    {
                        "message": f"Tool '{tool_id}' from decorated function was already loaded (e.g., as a class-based plugin via tool_configurations). Explicit/prior loading takes precedence."
                    },
                )
                continue
            ...
            should_enable_this_tool = auto_enable or (
                tool_id in self._tool_initial_configs
            )
            ...
            if should_enable_this_tool:
                ...
                if auto_enable:  # Logged as auto-enabled
                    await self._trace(
                        "log.info",
                        {
                            "message": f"Auto-enabled tool '{tool_id}' from decorated function '{func_item.__name__}'."
                        },
                    )
                    ...
                    await self._trace(
                        "log.info",
                        {
                            "message": f"Explicitly enabled tool '{tool_id}' from decorated function '{func_item.__name__}' via tool_configurations."
                        },
                    )
                ...
                await self._trace(
                    "log.warning",
                    {
                        "message": f"Tool '{tool_id}' from decorated function '{func_item.__name__}' was registered but is NOT active. To enable it, add '{tool_id}' to the `tool_configurations` dictionary in MiddlewareConfig."
                    },
                )
        ...
        if registered_count > 0 and not auto_enable:  # Log summary if any tools were enabled explicitly this way
            await self._trace(
                "log.info",
                {
                    "message": f"Explicitly enabled {registered_count} decorated tools listed in tool_configurations."
                },
            )
        elif registered_count > 0 and auto_enable:
            await self._trace(
                "log.info",
                {"message": f"Auto-enabled {registered_count} decorated tools."},
            )
    ...
    async def list_available_formatters(self) -> List[Dict[str, str]]:
        ...
        for (
            plugin_id,
            _plugin_class_obj,
        ) in self._plugin_manager.list_discovered_plugin_classes().items():
            try:
                instance = await self._plugin_manager.get_plugin_instance(
                    plugin_id, config={}
                )
                ...
                await self._trace(
                    "log.debug",
                    {
                        "message": f"Could not instantiate or check plugin '{plugin_id}' as DefinitionFormatter: {e}"
                    },
                )
    ...
    async def get_tool(self, identifier: str) -> Optional[Tool]:
        ...
        if not tool:
            await self._trace(
                "log.debug",
                {
                    "message": f"Tool with identifier '{identifier}' not found in ToolManager (not explicitly configured or loaded)."
                },
            )
        return tool
        ...
        for tool_instance in all_tools:
            try:
                ...
                summaries.append(
                    {
                        ...
                        "short_description": metadata.get(
                            "description_llm", metadata.get("description_human", "")
                        )[:120]
                        + "...",
                ...
                await self._trace(
                    "log.error",
                    {
                        "message": f"Error getting metadata for tool '{tool_instance.identifier}': {e}",
                        "exc_info": True,
                    },
                    level="error",
                )
            ...
            try:
                ...
                if page_size_val >= 1:
                    ...
                    await self._trace(
                        "log.debug",
                        {
                            "message": f"Invalid page_size '{page_size_input}', using default {default_page_size}."
                        },
                    )
                ...
                await self._trace(
                    "log.debug",
                    {
                        "message": f"Non-integer page_size '{page_size_input}', using default {default_page_size}."
                    },
                )
    ...
    async def get_formatted_tool_definition(
        self, tool_identifier: str, formatter_id: str
    ) -> Optional[Any]:
        tool = await self.get_tool(tool_identifier)
        if not tool:
            return None
        formatter_instance_any = await self._plugin_manager.get_plugin_instance(
            formatter_id
        )
        if not formatter_instance_any or not isinstance(
            formatter_instance_any, DefinitionFormatter
        ):
            await self._trace(
                "log.warning",
                {
                    "message": f"DefinitionFormatter plugin '{formatter_id}' not found or invalid."
                },
            )
            return None
        ...
        try:
            ...
            await self._trace(
                "log.error",
                {
                    "message": f"Error formatting tool '{tool_identifier}' with formatter '{formatter_id}': {e}",
                    "exc_info": True,
                },
                level="error",
            )
