...
class EmbeddingSimilarityLookupProvider(ToolLookupProvider):
    ...
    async def index_tools(self, tools_data: List[Dict[str, Any]], config: Optional[Dict[str, Any]] = None) -> None:
        ...
        texts_to_embed: List[str] = []; tool_info_for_indexing: List[Dict[str, Any]] = []
        for item_data in tools_data:
            text_repr = item_data.get("lookup_text_representation"); identifier = item_data.get("identifier")
            if isinstance(text_repr, str) and text_repr.strip() and identifier: texts_to_embed.append(text_repr); tool_info_for_indexing.append(item_data)
            else: logger.warning(f"{self.plugin_id}: Skipping tool data for indexing due to missing text or identifier: {str(item_data)[:100]}")
        if not texts_to_embed: logger.info(f"{self.plugin_id}: No valid tool texts provided for indexing."); return
        all_embeddings: List[EmbeddingVector] = []; embedding_to_tool_info_map: Dict[int, Dict[str, Any]] = {}
        async def _chunks_stream_for_embed(texts: List[str], infos: List[Dict[str,Any]]) -> AsyncIterable[Chunk]:
            for i, text_content in enumerate(texts): chunk_id_prefix = infos[i].get("identifier", f"tool_desc_{i}"); yield _LookupQueryChunk(text_content, id_prefix=f"{chunk_id_prefix}_idx_")
        embed_runtime_cfg = (config or {}).get("embedder_config", {}).copy()
        embed_runtime_cfg.setdefault("plugin_manager", self._plugin_manager)
        if self._key_provider and "key_provider" not in embed_runtime_cfg: embed_runtime_cfg["key_provider"] = self._key_provider
        try:
            ...
            async for chunk_obj, vec_list in self._embedder.embed(chunks=_chunks_stream_for_embed(texts_to_embed, tool_info_for_indexing), config=embed_runtime_cfg):
                if vec_list and isinstance(vec_list, list) and all(isinstance(x, float) for x in vec_list):
                    ...
                    if original_chunk_index < len(tool_info_for_indexing): embedding_to_tool_info_map[len(all_embeddings)-1] = tool_info_for_indexing[original_chunk_index]
                else: logger.warning(f"{self.plugin_id}: Invalid/empty embedding for content of chunk ID '{getattr(chunk_obj, 'id', 'N/A')}' (original index {original_chunk_index}). Skipped.")
                original_chunk_index += 1
            if len(all_embeddings) != len(texts_to_embed): logger.error(f"{self.plugin_id}: Mismatch after embedding. Expected {len(texts_to_embed)} embeddings, got {len(all_embeddings)}. Index may be incomplete."); tool_info_for_indexing = [embedding_to_tool_info_map[i] for i in range(len(all_embeddings)) if i in embedding_to_tool_info_map]
        except Exception as e: logger.error(f"{self.plugin_id}: Error during embedding tool texts: {e}", exc_info=True); return
        ...
        if self._tool_vector_store:
            async def _vs_stream_for_add() -> AsyncIterable[Tuple[Chunk, EmbeddingVector]]:
                for i, vec in enumerate(all_embeddings):
                    tool_info_item = tool_info_for_indexing[i]; vs_chunk = _LookupQueryChunk(tool_info_item["lookup_text_representation"], f"{tool_info_item['identifier']}_vs_"); vs_chunk.id = tool_info_item["identifier"]; vs_chunk.metadata = tool_info_item.get("_raw_metadata_snapshot", tool_info_item); yield vs_chunk, vec
            vs_add_cfg = (config or {}).get("vector_store_config", {}).copy()
            vs_add_cfg.setdefault("plugin_manager", self._plugin_manager)
            if "collection_name" not in vs_add_cfg: vs_add_cfg["collection_name"] = self._tool_embedding_collection_name
            try: add_result = await self._tool_vector_store.add(embeddings=_vs_stream_for_add(), config=vs_add_cfg); logger.info(f"{self.plugin_id}: Indexed {add_result.get('added_count',0)} tool embeddings to Vector Store '{self._tool_vector_store.plugin_id}'. Errors: {add_result.get('errors',[])}")
            except Exception as e_vs_add: logger.error(f"{self.plugin_id}: Error adding tool embeddings to Vector Store: {e_vs_add}", exc_info=True)
        elif np:
            try: self._indexed_tool_embeddings_np = np.array(all_embeddings, dtype=np.float32); self._indexed_tool_data_list_np = tool_info_for_indexing; logger.info(f"{self.plugin_id}: Indexed {len(self._indexed_tool_data_list_np)} tools using in-memory NumPy array (Shape: {self._indexed_tool_embeddings_np.shape}).")
            except ValueError as e_np: logger.error(f"{self.plugin_id}: Failed to convert embeddings to NumPy array: {e_np}. In-memory indexing failed.", exc_info=True); self._indexed_tool_embeddings_np = None; self._indexed_tool_data_list_np = []
        else: logger.error(f"{self.plugin_id}: Neither Vector Store nor NumPy is available for indexing tool embeddings.")
    ...
    async def find_tools(self, natural_language_query: str, top_k: int = 5, config: Optional[Dict[str,Any]]=None) -> List[RankedToolResult]:
        ...
        try:
            async for _chunk, vec_list in self._embedder.embed(chunks=_query_chunk_stream_for_embed(), config=embed_runtime_cfg):
                if vec_list and isinstance(vec_list, list) and all(isinstance(x, float) for x in vec_list): query_embedding = vec_list; break
        except Exception as e_q_embed: logger.error(f"{self.plugin_id}: Error embedding query '{natural_language_query[:50]}...': {e_q_embed}", exc_info=True); return []
        ...
        if self._tool_vector_store:
            ...
            if "collection_name" not in vs_search_cfg: vs_search_cfg["collection_name"] = self._tool_embedding_collection_name
            try: retrieved_vs_chunks: List[CoreRetrievedChunk] = await self._tool_vector_store.search(query_embedding, top_k, config=vs_search_cfg); return [RankedToolResult(tool_identifier=rc.id or f"vs_retrieved_{i}", score=rc.score, matched_tool_data=rc.metadata or {"id": rc.id, "text_content": rc.content}, description_snippet=rc.content[:200]) for i, rc in enumerate(retrieved_vs_chunks)]
            except Exception as e_vs_search: logger.error(f"{self.plugin_id}: Error searching Vector Store: {e_vs_search}", exc_info=True); return []
            ...
            query_np_array = np.array(query_embedding, dtype=np.float32).reshape(1, -1); index_embeddings_np = self._indexed_tool_embeddings_np
            if index_embeddings_np.ndim == 1:
                if index_embeddings_np.shape[0] == query_np_array.shape[1]: index_embeddings_np = index_embeddings_np.reshape(1, -1)
                else: logger.error(f"{self.plugin_id}: In-memory index has unexpected shape for a single item: {index_embeddings_np.shape}"); return []
            if index_embeddings_np.size == 0 or index_embeddings_np.shape[1] != query_np_array.shape[1]: logger.error(f"{self.plugin_id}: In-memory index/query dimension mismatch ({index_embeddings_np.shape} vs {query_np_array.shape}). Cannot compute similarity."); return []
            dot_products = np.dot(index_embeddings_np, query_np_array.T).flatten(); query_norm = np.linalg.norm(query_np_array); index_norms = np.linalg.norm(index_embeddings_np, axis=1)
            similarities = dot_products / ((index_norms * query_norm) + 1e-9); similarities = np.clip(similarities, -1.0, 1.0); similarities = np.nan_to_num(similarities, nan=-1.0)
            num_tools_in_index = len(self._indexed_tool_data_list_np); actual_top_k = min(top_k, num_tools_in_index)
            if actual_top_k == 0: return []
            top_k_indices = np.argsort(-similarities)[:actual_top_k]; results: List[RankedToolResult] = []
            for i in top_k_indices:
                if 0 <= i < num_tools_in_index: tool_data = self._indexed_tool_data_list_np[i]; score = (float(similarities[i]) + 1.0) / 2.0; results.append(RankedToolResult(tool_identifier=tool_data.get("identifier", f"unknown_tool_{i}"), score=score, matched_tool_data=tool_data, description_snippet=tool_data.get("lookup_text_representation","")[:200]))
            return results
        else: logger.warning(f"{self.plugin_id}: Tool index not built or NumPy not available for search."); return []

    async def teardown(self) -> None:
        self._embedder = None; self._tool_vector_store = None; self._plugin_manager = None; self._key_provider = None
        self._indexed_tool_embeddings_np = None; self._indexed_tool_data_list_np = []
