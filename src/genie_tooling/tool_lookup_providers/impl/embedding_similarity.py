...
class EmbeddingSimilarityLookupProvider(ToolLookupProvider):
    ...
    _indexed_tool_embeddings_np: Optional[Any] = None # Stores np.ndarray if using in-memory numpy
    _indexed_tool_data_list_np: List[Dict[str, Any]] = [] # Corresponding tool data for numpy index
    _tool_vector_store: Optional[VectorStorePlugin] = None
    _tool_embedding_collection_name: str = "genie_tool_embeddings_v1" # Default, can be overridden
    ...
    DEFAULT_VECTOR_STORE_ID_FOR_TOOL_LOOKUP = "chromadb_vector_store_v1" # Example default if VS is used
    ...
    async def setup(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self._key_provider = cfg.get("key_provider") # Passed from ToolLookupService
        ...
        if vector_store_id:
            ...
            # Prioritize direct config for collection name and path for this specific tool lookup VS instance
            self._tool_embedding_collection_name = cfg.get("tool_embeddings_collection_name",
                                                           vs_config_for_setup.get("collection_name", self._tool_embedding_collection_name))
            vs_config_for_setup["collection_name"] = self._tool_embedding_collection_name

            tool_embeddings_path = cfg.get("tool_embeddings_path") # Path from direct config
            if tool_embeddings_path is not None: # If explicitly provided (even if None to force ephemeral for some VS)
                vs_config_for_setup["path"] = tool_embeddings_path
            # If not directly provided, VS plugin's own default path logic (if any) will apply based on its setup.

            if self._key_provider and "key_provider" not in vs_config_for_setup:
                 vs_config_for_setup["key_provider"] = self._key_provider # Pass KP to VS if needed
    ...
    async def index_tools(self, tools_data: List[Dict[str, Any]], config: Optional[Dict[str, Any]] = None) -> None:
        ...
        for item_data in tools_data:
            ...
            if isinstance(text_repr, str) and text_repr.strip() and identifier:
                ...
                tool_info_for_indexing.append(item_data) # Store the whole formatted item
            else:
        ...
        async def _chunks_stream_for_embed(texts: List[str], infos: List[Dict[str,Any]]) -> AsyncIterable[Chunk]:
            for i, text_content in enumerate(texts):
                # Use the tool's identifier for the chunk ID if possible for better traceability
        ...
        if self._key_provider and "key_provider" not in embed_runtime_cfg: # Pass KP to embedder if needed
            embed_runtime_cfg["key_provider"] = self._key_provider

        try:
            original_chunk_index = 0
            async for chunk_obj, vec_list in self._embedder.embed(
                chunks=_chunks_stream_for_embed(texts_to_embed, tool_info_for_indexing),
                config=embed_runtime_cfg
            ):
                if vec_list and isinstance(vec_list, list) and all(isinstance(x, float) for x in vec_list):
                    all_embeddings.append(vec_list)
                    # Link this embedding back to its original tool_info item
            ...
            if len(all_embeddings) != len(texts_to_embed):
                ...
                # Rebuild tool_info_for_indexing to match successfully generated embeddings
        ...
        if self._tool_vector_store:
            async def _vs_stream_for_add() -> AsyncIterable[Tuple[Chunk, EmbeddingVector]]:
                for i, vec in enumerate(all_embeddings):
                    # Use the tool_info that corresponds to this embedding
                    tool_info_item = tool_info_for_indexing[i]
                    # Create a Chunk for the VS, using tool identifier as ID
                    vs_chunk = _LookupQueryChunk(tool_info_item["lookup_text_representation"], f"{tool_info_item['identifier']}_vs_")
                    vs_chunk.id = tool_info_item["identifier"] # Crucial: store original tool ID
                    vs_chunk.metadata = tool_info_item.get("_raw_metadata_snapshot", tool_info_item) # Store rich metadata
                    yield vs_chunk, vec

            vs_add_cfg = (config or {}).get("vector_store_config", {})
            if "collection_name" not in vs_add_cfg: # Ensure collection name is passed
        ...
        elif np: # Fallback to NumPy in-memory index
            try:
                self._indexed_tool_embeddings_np = np.array(all_embeddings, dtype=np.float32)
                self._indexed_tool_data_list_np = tool_info_for_indexing # Already filtered to match embeddings
    ...
    async def find_tools(self, natural_language_query: str, top_k: int = 5, config: Optional[Dict[str,Any]]=None) -> List[RankedToolResult]:
        ...
        if self._tool_vector_store:
            ...
            try:
                ...
                return [RankedToolResult(
                            tool_identifier=rc.id or f"vs_retrieved_{i}", # Use chunk ID from VS (should be tool_id)
                            score=rc.score,
                            matched_tool_data=rc.metadata or {"id": rc.id, "text_content": rc.content}, # Metadata from VS
                            description_snippet=rc.content[:200] # Use content as snippet
                ...
                return []
        elif np and self._indexed_tool_embeddings_np is not None and \
             hasattr(self._indexed_tool_embeddings_np, "shape") and \
             self._indexed_tool_embeddings_np.shape and self._indexed_tool_embeddings_np.size > 0:

            query_np_array = np.array(query_embedding, dtype=np.float32).reshape(1, -1)
            index_embeddings_np = self._indexed_tool_embeddings_np

            if index_embeddings_np.ndim == 1: # Handle case where only one tool was indexed
            ...
            # Add epsilon to avoid division by zero if norms are zero
            similarities = dot_products / ((index_norms * query_norm) + 1e-9)
            similarities = np.clip(similarities, -1.0, 1.0) # Ensure valid range for cosine
            similarities = np.nan_to_num(similarities, nan=-1.0) # Handle potential NaNs
            ...
            for i in top_k_indices:
                if 0 <= i < num_tools_in_index: # Boundary check
                    tool_data = self._indexed_tool_data_list_np[i]
                    score = (float(similarities[i]) + 1.0) / 2.0 # Normalize cosine sim (-1 to 1) to score (0 to 1)
                    results.append(RankedToolResult(
                        tool_identifier=tool_data.get("identifier", f"unknown_tool_{i}"),
                        score=score,
                        matched_tool_data=tool_data, # The formatted data used for indexing
