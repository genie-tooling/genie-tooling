...
class LlamaCppInternalLLMProviderPlugin(LLMProviderPlugin):
    ...
    _model_path: str
    _model_name_for_logging: str = "local_llama_cpp_model" # For token usage, etc.
    _key_provider: Optional[KeyProvider] = None # Part of interface, not used here
    ...
    async def setup(self, config: Optional[Dict[str, Any]]) -> None:
        ...
        self._seed = int(cfg.get("seed", -1)) # -1 for random seed in llama-cpp
        ...
        try:
            ...
            # Model loading is blocking, run in executor
            loop = asyncio.get_running_loop()
            self._model_client = await loop.run_in_executor(
                ...
                functools.partial(
                    ...
                    tensor_split=self._tensor_split,
                    seed=self._seed,
                    verbose=self._verbose_llama_cpp,
                    chat_format=self._chat_format, # Pass chat_format here for Llama instance
                    lora_path=self._lora_path,
                    lora_base=self._lora_base,
        ...
        if stream:
            async def stream_generator() -> AsyncIterable[LLMCompletionChunk]:
                completion_stream = await loop.run_in_executor(
                    None,
                    functools.partial(
                        self._model_client.create_completion, # type: ignore
                        prompt=prompt,
                        stream=True,
                        **gen_params
                    )
            ...
            return {
                "text": text_content, "finish_reason": finish_reason,
                "usage": usage_info, "raw_response": completion
            }
                    ...
                    if "tool_calls" in delta_raw and delta_raw["tool_calls"]:
                        ...
                        for tc_delta in delta_raw["tool_calls"]:
                            if tc_delta.get("type") == "function" and tc_delta.get("function"):
                                genie_tool_calls_delta.append({
                                    "id": tc_delta.get("id", f"call_{tc_delta.get('index', 'unk')}_{asyncio.get_event_loop().time()}"),
                                    "type": "function",
                                    "function": {
