...
class LlamaCppInternalLLMProviderPlugin(LLMProviderPlugin):
    ...
    async def generate(self, prompt: str, stream: bool = False, **kwargs: Any) -> Union[LLMCompletionResponse, AsyncIterable[LLMCompletionChunk]]:
        ...
        if output_schema and LlamaGrammar:
            try:
                ...
                if isinstance(output_schema, type) and issubclass(output_schema, PydanticBaseModel): gbnf_grammar_str = generate_gbnf_grammar_from_pydantic_models([output_schema])
                elif isinstance(output_schema, dict): dynamic_models = create_dynamic_models_from_dictionaries([output_schema]); gbnf_grammar_str = generate_gbnf_grammar_from_pydantic_models(dynamic_models) if dynamic_models else None
                if gbnf_grammar_str: gen_params["grammar"] = LlamaGrammar.from_string(gbnf_grammar_str); logger.info(f"{self.plugin_id}: Using GBNF grammar for structured output via generate().")
            except Exception as e_gbnf: logger.error(f"{self.plugin_id}: Failed to generate or apply GBNF grammar: {e_gbnf}", exc_info=True)
        loop = asyncio.get_running_loop()
        if stream:
            async def stream_generator() -> AsyncIterable[LLMCompletionChunk]:
                ...
                full_text = ""; final_usage_info: Optional[LLMUsageInfo] = None
                for chunk_data in completion_stream: # type: ignore
                    text_delta = chunk_data["choices"][0].get("text", ""); full_text += text_delta; finish_reason_str = chunk_data["choices"][0].get("finish_reason")
                    current_chunk: LLMCompletionChunk = {"text_delta": text_delta, "raw_chunk": chunk_data}
                    if finish_reason_str: current_chunk["finish_reason"] = finish_reason_str; raw_usage = chunk_data.get("usage"); final_usage_info = {"prompt_tokens": raw_usage.get("prompt_tokens"), "completion_tokens": raw_usage.get("completion_tokens"), "total_tokens": raw_usage.get("total_tokens")} if raw_usage else None; current_chunk["usage_delta"] = final_usage_info
                    yield current_chunk
            ...
            text_content = completion["choices"][0].get("text", ""); finish_reason = completion["choices"][0].get("finish_reason", "unknown"); usage_data = completion.get("usage"); usage_info: Optional[LLMUsageInfo] = None
            if usage_data: usage_info = {"prompt_tokens": usage_data.get("prompt_tokens"), "completion_tokens": usage_data.get("completion_tokens"), "total_tokens": usage_data.get("total_tokens")}
    ...
    async def chat(self, messages: List[ChatMessage], stream: bool = False, **kwargs: Any) -> Union[LLMChatResponse, AsyncIterable[LLMChatChunk]]:
        ...
        if output_schema and LlamaGrammar:
            try:
                ...
                if gbnf_grammar_str: chat_params["grammar"] = LlamaGrammar.from_string(gbnf_grammar_str); logger.info(f"{self.plugin_id}: Using GBNF grammar for structured output via chat().")
            except Exception as e_gbnf: logger.error(f"{self.plugin_id}: Failed to generate or apply GBNF grammar for chat: {e_gbnf}", exc_info=True)
        loop = asyncio.get_running_loop()
        if stream:
            async def stream_chat_chunks() -> AsyncIterable[LLMChatChunk]:
                ...
                for chunk_data in completion_stream: # type: ignore
                    ...
                    if "tool_calls" in delta_raw and delta_raw["tool_calls"]:
                        ...
                        if genie_tool_calls_delta: delta_message["tool_calls"] = genie_tool_calls_delta
                    current_chunk: LLMChatChunk = {"message_delta": delta_message, "raw_chunk": chunk_data}; finish_reason_str = chunk_data["choices"][0].get("finish_reason")
                    if finish_reason_str: current_chunk["finish_reason"] = finish_reason_str; raw_usage = chunk_data.get("usage"); final_usage_info = {"prompt_tokens": raw_usage.get("prompt_tokens"), "completion_tokens": raw_usage.get("completion_tokens"), "total_tokens": raw_usage.get("total_tokens")} if raw_usage else None; current_chunk["usage_delta"] = final_usage_info
                    yield current_chunk
            ...
            choice = completion["choices"][0]; raw_assistant_message = choice["message"]
            assistant_message: ChatMessage = {"role": cast(Any, raw_assistant_message.get("role", "assistant"))}
            if raw_assistant_message.get("content") is not None: assistant_message["content"] = raw_assistant_message.get("content")
            if raw_assistant_message.get("tool_calls"):
                ...
                for tc_raw in raw_assistant_message["tool_calls"]:
                    if tc_raw.get("type") == "function" and tc_raw.get("function"): genie_tool_calls_list.append({"id": tc_raw.get("id", str(asyncio.get_event_loop().time())), "type": "function", "function": {"name": tc_raw["function"].get("name", "unknown_function"), "arguments": tc_raw["function"].get("arguments", "{}")}})
                if genie_tool_calls_list: assistant_message["tool_calls"] = genie_tool_calls_list
            finish_reason = choice.get("finish_reason", "unknown"); usage_data = completion.get("usage"); usage_info: Optional[LLMUsageInfo] = None
            if usage_data: usage_info = {"prompt_tokens": usage_data.get("prompt_tokens"), "completion_tokens": usage_data.get("completion_tokens"), "total_tokens": usage_data.get("total_tokens")}
    ...
    async def get_model_info(self) -> Dict[str, Any]:
        ...
        if self._model_client and hasattr(self._model_client, "model_params"):
            try: info["llama_cpp_model_params_available"] = True
            except Exception: info["llama_cpp_model_params_available"] = False
        return info

    async def teardown(self) -> None:
        if self._model_client: logger.info(f"{self.plugin_id}: Releasing Llama model '{self._model_path}'."); del self._model_client; self._model_client = None
        self._key_provider = None; self._token_usage_manager = None
