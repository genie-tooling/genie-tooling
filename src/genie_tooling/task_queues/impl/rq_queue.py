### src/genie_tooling/task_queues/impl/rq_queue.py
import asyncio
import functools
import logging
...
class RedisQueueTaskPlugin(DistributedTaskQueuePlugin):
    ...
    _redis_conn: Optional[Any] = None # redis.Redis
    _queues: Dict[str, Any] = {} # Dict[str, rq.Queue]
    ...
    async def setup(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        try:
            self._redis_conn = Redis.from_url(redis_url) # type: ignore
            ...
            logger.info(f"{self.plugin_id}: Connected to Redis for RQ at {redis_url}. Default queue: '{self._default_queue_name}'.")
    ...
    async def get_task_result(
        ...
        try:
            ...
            if timeout_seconds is not None and timeout_seconds > 0:
                ...
                while not await loop.run_in_executor(None, lambda j=job: j.is_finished) and \
                      not await loop.run_in_executor(None, lambda j=job: j.is_failed) and \
                      not await loop.run_in_executor(None, lambda j=job: j.is_canceled):
                    if (asyncio.get_event_loop().time() - start_time) > timeout_seconds:
                        raise TimeoutError(f"Timeout waiting for RQ task '{task_id}' result.")
                    await asyncio.sleep(0.1)

            if await loop.run_in_executor(None, lambda j=job: j.is_failed): # type: ignore
                exc_info = await loop.run_in_executor(None, lambda j=job: j.exc_info)
                raise RuntimeError(f"RQ task '{job.id}' failed. Info: {exc_info}")
            if await loop.run_in_executor(None, lambda j=job: j.is_canceled): # type: ignore
                raise RuntimeError(f"RQ task '{job.id}' was canceled.")

            return await loop.run_in_executor(None, lambda j=job: j.result)
        except NoSuchJobError: # type: ignore
            raise KeyError(f"RQ Task ID '{task_id}' not found.") from None
        except Exception as e:
    ...
    async def revoke_task(self, task_id: str, queue_name: Optional[str] = None, terminate: bool = False) -> bool:
        ...
        try:
            ...
            if await loop.run_in_executor(None, lambda j=job: j.is_started) and terminate: # type: ignore
                ...
                return True
            elif not await loop.run_in_executor(None, lambda j=job: j.is_finished) and \
                 not await loop.run_in_executor(None, lambda j=job: j.is_failed) and \
                 not await loop.run_in_executor(None, lambda j=job: j.is_canceled): # type: ignore
                await loop.run_in_executor(None, job.cancel) # type: ignore
