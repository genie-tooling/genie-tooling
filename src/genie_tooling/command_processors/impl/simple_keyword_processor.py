...
class SimpleKeywordToolSelectorProcessorPlugin(CommandProcessorPlugin):
    ...
    async def _prompt_for_param(self, genie: "Genie", param_name: str, param_schema: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            await genie.observability.trace_event("log.warning", {"message": f"Could not coerce input '{user_input_str}' to type '{param_type}' for param '{param_name}'. Returning as string."}, "SimpleKeywordToolSelectorProcessor")
            return user_input_str

    async def process_command(self, command: str, conversation_history: Optional[List[ChatMessage]] = None, correlation_id: Optional[str] = None, genie_instance: Optional["Genie"] = None) -> CommandProcessorResponse:
        ...
        for keyword in keywords_to_check:
            if keyword.lower() in command_lower:
                ...
                await genie_to_use.observability.trace_event("log.info", {"message": f"Matched keyword '{keyword}', selected tool '{chosen_tool_id}'."}, "SimpleKeywordToolSelectorProcessor", correlation_id)
                break
        if not chosen_tool_id:
            await genie_to_use.observability.trace_event("log.info", {"message": f"No keyword match found in command: '{command}'."}, "SimpleKeywordToolSelectorProcessor", correlation_id)
            return {"llm_thought_process": "No matching keywords found for any configured tool."}
        if not hasattr(genie_to_use, "_tool_manager") or not genie_to_use._tool_manager: # type: ignore
            await genie_to_use.observability.trace_event("log.error", {"message": "ToolManager not available via Genie facade."}, "SimpleKeywordToolSelectorProcessor", correlation_id)
            return {"error": "Internal error: ToolManager not accessible."}
        tool_instance: Optional[Tool] = await genie_to_use._tool_manager.get_tool(chosen_tool_id) # type: ignore
        if not tool_instance:
            await genie_to_use.observability.trace_event("log.error", {"message": f"Selected tool '{chosen_tool_id}' not found in ToolManager."}, "SimpleKeywordToolSelectorProcessor", correlation_id)
        ...
        try:
            ...
            for param_name, param_schema_val in properties.items():
                ...
                if param_name in required_params or (await asyncio.to_thread(input, f"  Provide optional parameter '{param_name}' ({param_schema_dict.get('type', 'any')})? (y/N): ")).lower() == "y":
                    try:
                        extracted_params[param_name] = await self._prompt_for_param(genie_to_use, param_name, param_schema_dict)
            ...
            await genie_to_use.observability.trace_event("log.error", {"message": f"Error while getting schema or prompting for tool '{chosen_tool_id}': {e}", "exc_info": True}, "SimpleKeywordToolSelectorProcessor", correlation_id)
