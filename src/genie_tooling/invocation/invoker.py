...
class ToolInvoker:
    def __init__(self, tool_manager: ToolManager, plugin_manager: PluginManager, default_strategy_id: str = DEFAULT_STRATEGY_ID):
        self._tool_manager = tool_manager; self._plugin_manager = plugin_manager; self._default_strategy_id = default_strategy_id
    ...
    async def invoke(self, tool_identifier: str, params: Dict[str, Any], key_provider: KeyProvider, context: Optional[Dict[str, Any]] = None, strategy_id: Optional[str] = None, validator_id: Optional[str] = None, transformer_id: Optional[str] = None, error_handler_id: Optional[str] = None, error_formatter_id: Optional[str] = None, cache_provider_id: Optional[str] = None, cache_config: Optional[Dict[str, Any]] = None, invoker_config: Optional[Dict[str, Any]] = None) -> Any:
        current_invoker_config = invoker_config or {}; tracing_manager: Optional[InteractionTracingManager] = current_invoker_config.get("tracing_manager"); guardrail_manager: Optional[GuardrailManager] = current_invoker_config.get("guardrail_manager"); correlation_id: Optional[str] = current_invoker_config.get("correlation_id")
        async def _trace(event_name: str, data: Dict):
            if tracing_manager: await tracing_manager.trace_event(event_name, data, f"ToolInvoker:{tool_identifier}", correlation_id)
        ...
        tool_manager_instance = self._tool_manager; tool: Optional[Tool] = await tool_manager_instance.get_tool(tool_identifier) # type: ignore
        if not tool:
            error_msg = f"Tool '{tool_identifier}' not found."; logger.warning(error_msg); await _trace("tool_invoker.invoke.tool_not_found", {"error": error_msg})
        ...
        try:
            ...
            error_msg = f"Critical unhandled error during strategy execution for tool '{tool_identifier}': {str(e)}"; logger.critical(error_msg, exc_info=True)
            await _trace("tool_invoker.invoke.strategy_error", {"error": error_msg, "exception_type": type(e).__name__})
            structured_error: StructuredError = {"type": "InternalExecutionError", "message": error_msg, "details": {"tool_id": tool_identifier, "strategy_id": chosen_strategy_id}}; formatter = await self._get_default_error_formatter()
            return formatter.format(structured_error) if formatter else structured_error
    async def teardown(self) -> None: logger.info("ToolInvoker teardown (no specific resources to release here)."); pass
