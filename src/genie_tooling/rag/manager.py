...
class RAGManager:
    ...
    async def _trace(self, event_name: str, data: Dict, level: str = "info"):
        if self._tracing_manager:
            await self._tracing_manager.trace_event(f"log.{level}", {"message": event_name, **data}, "RAGManager")
    ...
    async def _get_plugin_instance_for_rag(
        ...
        if instance_any and isinstance(instance_any, expected_protocol):
            await self._trace("log.debug", {"message": f"Successfully instantiated and set up {component_name} plugin '{plugin_id}'."})
            return cast(PT, instance_any)

        if instance_any:
            await self._trace("log.error", {"message": f"Instantiated plugin '{plugin_id}' is not a valid {component_name}. Type: {type(instance_any)}"})
        else:
            await self._trace("log.error", {"message": f"{component_name} plugin '{plugin_id}' not found or failed to load."})
        return None
    ...
    async def index_data_source(
        ...
        await self._trace("log.info", {"message": f"Starting RAG indexing for source '{loader_source_uri}' using pipeline: Loader='{loader_id}', Splitter='{splitter_id}', Embedder='{embedder_id}', Store='{vector_store_id}'."})
    ...
    async def retrieve_from_query(self, query_text: str, retriever_id: str, retriever_config: Optional[Dict[str, Any]] = None, top_k: int = 5) -> List[RetrievedChunk]:
        ...
        if not retriever_plugin:
            await self._trace("log.error", {"message": f"Retriever plugin '{retriever_id}' not found or failed to load."})
            return []
        ...
        try:
            ...
            await self._trace("log.info", {"message": f"Retrieved {len(results)} chunks for query using retriever '{retriever_id}'."})
            return results
        except Exception as e:
            await self._trace("log.error", {"message": f"Error during RAG retrieval with retriever '{retriever_id}': {e}", "exc_info": True})
            return []
