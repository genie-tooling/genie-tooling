...
from genie_tooling.rag.plugins.abc import DocumentLoaderPlugin
...
class FileSystemLoader(DocumentLoaderPlugin):
    ...
    async def load(self, source_uri: str, config: Optional[Dict[str, Any]] = None) -> AsyncIterable[Document]:
        ...
        if not directory.is_dir():
            ...
            # print(f"[LOADER DEBUG] Directory {directory} is not valid.") # TEMP DEBUG
            if False:
                yield
                return
        ...
        print(f"[LOADER DEBUG] About to rglob in {directory} with pattern {glob_pattern}") # TEMP DEBUG
        found_files_for_debug = list(directory.rglob(glob_pattern))
        print(f"[LOADER DEBUG] rglob found: {[str(f.resolve()) for f in found_files_for_debug]}") # TEMP DEBUG

        for file_path in directory.rglob(glob_pattern): # Use original rglob here
            print(f"[LOADER DEBUG] Processing file_path from rglob: {file_path.resolve()}") # TEMP DEBUG
            file_count += 1
            if file_path.is_file():
                print(f"[LOADER DEBUG] {file_path.resolve()} is a file.") # TEMP DEBUG
                try:
                    file_size = file_path.stat().st_size
                    if file_size == 0:
                        logger.debug(f"FileSystemLoader: Skipping empty file {file_path}.")
                        print(f"[LOADER DEBUG] Skipping empty file {file_path.resolve()}") # TEMP DEBUG
                        continue
                    if file_size > max_file_size_bytes:
                        ...
                        print(f"[LOADER DEBUG] Skipping large file {file_path.resolve()}") # TEMP DEBUG
                        continue

                    print(f"[LOADER DEBUG] Attempting aiofiles.open for {file_path.resolve()}") # TEMP DEBUG
                    async with aiofiles.open(file_path, mode="r", encoding=encoding, errors="replace") as f:
                        content = await f.read()
                    print(f"[LOADER DEBUG] Successfully read {file_path.resolve()}") # TEMP DEBUG
                    ...
                    loaded_count +=1
                    print(f"[LOADER DEBUG] Yielding doc for {doc_id}") # TEMP DEBUG
                    ...
                    print(f"[LOADER DEBUG] Exception for {file_path.resolve()}: {e}") # TEMP DEBUG
            else:
                logger.debug(f"FileSystemLoader: Path {file_path} matched glob but is not a file. Skipping.")
                print(f"[LOADER DEBUG] {file_path.resolve()} is not a file, skipping.") # TEMP DEBUG
        logger.info(f"FileSystemLoader: Scanned {file_count} paths, loaded {loaded_count} documents from '{source_uri}'.")
        print(f"[LOADER DEBUG] Load method finished. Loaded_count: {loaded_count}") # TEMP DEBUG
