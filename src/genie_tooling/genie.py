...
class Genie:
    def __init__(
        ...
        self._key_provider = key_provider
        self._config = config # This is the RESOLVED config
        ...
        self._config._genie_instance = self
        logger.info("Genie facade initialized with resolved configuration.")

    @classmethod
    async def create(
        cls,
        config: MiddlewareConfig, # User-provided config
        ...
        # Main PluginManager uses resolved_config for dev_dirs
        pm = PluginManager(plugin_dev_dirs=resolved_config.plugin_dev_dirs)
        await pm.discover_plugins()

        # Ensure the actual_key_provider (if plugin) is known by the main PM by its instance
        if isinstance(actual_key_provider, CorePluginType):
            ...
            if kp_main_pm_plugin_id and (kp_main_pm_plugin_id not in pm._plugin_instances or pm._plugin_instances[kp_main_pm_plugin_id] is not actual_key_provider): # type: ignore
                 ...
                 # Optionally, if it wasn't discovered via entry points or main dev_dirs
        ...
        # All managers initialized with the fully resolved_config and the single actual_key_provider
