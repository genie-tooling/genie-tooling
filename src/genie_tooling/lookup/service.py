...
class ToolLookupService:
    ...
    async def _trace(self, event_name: str, data: Dict, level: str = "info", correlation_id: Optional[str] = None):
        if self._tracing_manager:
            ...
            # FIX: Call trace_event positionally to match test expectations.
            ...
            log_func(f"{event_name} | {data.get('message', str(data))}")
    ...
    async def _get_formatted_tool_data(self, tool_id: str, formatter_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not indexing_formatter_any or not isinstance(indexing_formatter_any, DefinitionFormatter):
            await self._trace("log.error", {"message": f"Indexing formatter plugin '{formatter_id}' not found or invalid."})
            return None
        ...
        if not tool_instance:
            await self._trace("log.warning", {"message": f"Tool '{tool_id}' not found in ToolManager for formatting."})
            return None
        ...
        try:
            ...
            if isinstance(formatted_data, dict):
                ...
                await self._trace("log.warning", {"message": f"Formatter '{formatter_id}' for tool '{tool_id}' produced unexpected type: {type(formatted_data)}. Skipping."})
                return None
        except Exception as e_format:
            await self._trace("log.error", {"message": f"Error formatting tool '{tool_id}' for indexing with '{formatter_id}': {e_format}", "exc_info": True})
            return None
    ...
    async def _ensure_provider_is_indexed(self, provider_id: str, formatter_id: str, provider_config: Optional[Dict[str, Any]]):
        if not self._is_indexed_map.get(provider_id, False):
            await self._trace("log.info", {"message": f"Index for provider '{provider_id}' not built or invalidated. Triggering full re-index."})
    ...
    async def reindex_all_tools(self, provider_id: str, formatter_id: Optional[str] = None, provider_config: Optional[Dict[str, Any]] = None) -> bool:
        ...
        if not isinstance(provider_instance, ToolLookupProviderPlugin):
            await self._trace("log.error", {"message": f"Provider '{provider_id}' not found or invalid for reindexing."})
        ...
        if not formatter_id_to_use:
            await self._trace("log.error", {"message": f"No indexing formatter ID available for provider '{provider_id}'. Cannot index."})
            return False
        ...
        if not tools_data_for_indexing:
            await self._trace("log.warning", {"message": f"No tool data could be formatted for provider '{provider_id}'. Index will be empty."})
        ...
        try:
            ...
            await self._trace("log.info", {"message": f"Successfully re-indexed all {len(tools_data_for_indexing)} tools for provider '{provider_id}'."})
            return True
        except Exception as e_index:
            await self._trace("log.error", {"message": f"Error during full re-indexing with provider '{provider_id}': {e_index}", "exc_info": True})
            self._is_indexed_map[provider_id] = False
            return False
    ...
    async def find_tools(self, natural_language_query: str, top_k: int = 5, provider_id_override: Optional[str] = None, indexing_formatter_id_override: Optional[str] = None, provider_config_override: Optional[Dict[str, Any]] = None) -> List[RankedToolResult]:
        if not natural_language_query or not natural_language_query.strip():
            await self._trace("log.debug", {"message": "Empty query provided. Returning no results."})
            return []

        target_provider_id = provider_id_override or self._default_provider_id
        if not target_provider_id:
            await self._trace("log.warning", {"message": "No tool lookup provider specified or configured. Cannot find tools."})
            return []
            
        formatter_id_to_use = indexing_formatter_id_override or self._default_indexing_formatter_id
        if not formatter_id_to_use:
            await self._trace("log.error", {"message": f"No indexing formatter ID available for provider '{target_provider_id}'. Cannot ensure index."})
            return []
        ...
        if not isinstance(provider_instance, ToolLookupProviderPlugin):
            await self._trace("log.error", {"message": f"Tool lookup provider '{target_provider_id}' not found or invalid."})
            return []
        ...
        try:
            ...
            await self._trace("log.error", {"message": f"Error finding tools with provider '{target_provider_id}': {e_find}", "exc_info": True})
            return []

    async def invalidate_all_indices(self) -> None:
        await self._trace("log.info", {"message": "All provider indices invalidated."})
