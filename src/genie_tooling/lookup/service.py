...
class ToolLookupService:
    ...
    async def _get_formatted_tool_data_for_provider(
        ...
        formatter_config = {"plugin_manager": self._plugin_manager} # Pass PM to formatter
        indexing_formatter_any = await self._plugin_manager.get_plugin_instance(formatter_id_to_use, config=formatter_config)
        ...
        if not tools: logger.debug("ToolLookupService: No tools available to format for indexing."); return []
            ...
            try:
                ...
                if isinstance(formatted_data_item, dict):
                    ...
                    if "lookup_text_representation" not in formatted_data_item:
                         desc_llm = raw_metadata.get("description_llm", ""); desc_human = raw_metadata.get("description_human", ""); name = raw_metadata.get("name", tool_instance.identifier)
                    ...
                    formatted_tool_data_list.append({"identifier": tool_instance.identifier, "lookup_text_representation": formatted_data_item, "_raw_metadata_snapshot": {k:v for k,v in raw_metadata.items() if isinstance(v, (str,int,float,bool,list,dict,type(None)))}})
                else: logger.warning(f"Formatter '{formatter_id_to_use}' for tool '{tool_instance.identifier}' produced unexpected type: {type(formatted_data_item)}. Skipping.")
            except Exception as e_format: logger.error(f"Error formatting tool '{tool_instance.identifier}' for indexing with '{formatter_id_to_use}': {e_format}", exc_info=True)
    ...
    async def reindex_tools_for_provider(self, provider_id: str, formatter_plugin_id_override: Optional[str] = None, provider_config: Optional[Dict[str,Any]] = None) -> bool:
        ...
        if not provider_instance_any or not isinstance(provider_instance_any, ToolLookupProviderPlugin):
            logger.error(f"ToolLookupService: Provider '{provider_id}' not found or invalid for reindexing."); self._index_validity_map[provider_id] = False; return False
        ...
        if tools_data_for_indexing is None: logger.error(f"Failed to prepare tool data for provider '{provider_id}' due to formatter error. Indexing aborted."); self._index_validity_map[provider_id] = False; return False
        try:
            await provider_instance.index_tools(tools_data=tools_data_for_indexing, config=provider_setup_config)
            self._indexed_tools_data_cache[provider_id] = tools_data_for_indexing; self._index_validity_map[provider_id] = True
            logger.info(f"Successfully re-indexed tools for provider '{provider_id}'. Indexed {len(tools_data_for_indexing)} items.")
            return True
        except Exception as e_index: logger.error(f"Error during reindexing with provider '{provider_id}': {e_index}", exc_info=True); self._index_validity_map[provider_id] = False; return False
    ...
    async def find_tools(self, natural_language_query: str, top_k: int = 5, provider_id_override: Optional[str] = None, indexing_formatter_id_override: Optional[str] = None, provider_config_override: Optional[Dict[str, Any]] = None) -> List[RankedToolResult]:
        if not natural_language_query or not natural_language_query.strip(): logger.debug("ToolLookupService: Empty query provided. Returning no results."); return []
        ...
        if not provider_instance_any or not isinstance(provider_instance_any, ToolLookupProviderPlugin): logger.error(f"Tool lookup provider '{target_provider_id}' not found or invalid."); return []
        provider_instance = cast(ToolLookupProviderPlugin, provider_instance_any)
        if not self._index_validity_map.get(target_provider_id, False):
            ...
            reindex_successful = await self.reindex_tools_for_provider(target_provider_id, indexing_formatter_id_override, provider_config=current_provider_config)
            if not reindex_successful: logger.error(f"Re-indexing failed for '{target_provider_id}'. Cannot perform lookup."); return []
        try: return await provider_instance.find_tools(natural_language_query=natural_language_query, top_k=top_k, config=current_provider_config)
        except Exception as e_find: logger.error(f"Error finding tools with provider '{target_provider_id}': {e_find}", exc_info=True); return []

    def invalidate_index(self, provider_id: Optional[str] = None) -> None:
        if provider_id:
            if provider_id in self._index_validity_map: self._index_validity_map[provider_id] = False; logger.info(f"ToolLookupService: Index for provider '{provider_id}' invalidated.")
        else:
            for pid in list(self._index_validity_map.keys()): self._index_validity_map[pid] = False
