...
from .types import (  # Assuming ReActObservation is defined in types.py
    AgentOutput,
    ReActObservation,
)
...
DEFAULT_REACT_SYSTEM_PROMPT_ID = "react_agent_system_prompt_v1" # ID for a prompt in the registry

class ReActAgent(BaseAgent):
    """
    Implements the ReAct (Reason-Act) agentic loop.
    The agent iteratively reasons about the goal, decides on an action (tool use),
    executes the action, observes the result, and repeats until the goal is achieved
    ...
    def _parse_llm_reason_act_output(self, llm_output: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        """
        Parses the LLM output to extract Thought, Action, and potentially a final Answer.
        Expected format:
        Thought: [LLM's thought process]
        Action: ToolName[{"param1": "value1", ...}]  OR  Answer: [Final answer to the goal]
        """
        ...
        if action_match:
            ...
            # Ensure params are valid JSON, or at least try to make them
            if not (action_params_json.startswith("{") and action_params_json.endswith("}")):
                if not action_params_json: # Empty params
                    action_params_json = "{}"
                else: # Attempt to fix if it's just missing braces
                    logger.warning(f"ReActAgent: Action params '{action_params_json}' not a valid JSON object string. Attempting to wrap.")
        ...
        if not thought and not (action_tool_name or final_answer):
            logger.warning(f"ReActAgent: Could not parse Thought, Action, or Answer from LLM output: {llm_output[:200]}...")
            # Fallback: treat the whole output as a thought if nothing else is parsable.
    ...
    async def run(self, goal: str, **kwargs: Any) -> AgentOutput:
        ...
        for i in range(self.max_iterations):
            ...
            await self.genie.observability.trace_event("react_agent.iteration.start", {"iteration": i+1, "goal": goal, "scratchpad_size": len(scratchpad)}, "ReActAgent")

            # 1. Reason
            prompt_data: Dict[str, Any] = {
                "goal": goal,
                "scratchpad": "\n".join([f"Thought: {s['thought']}\nAction: {s['action']}\nObservation: {s['observation']}" for s in scratchpad]),
            ...
            for attempt in range(self.llm_retry_attempts + 1):
                try:
                    ...
                    if attempt < self.llm_retry_attempts:
                        ...
                        logger.error(f"ReActAgent: LLM call failed after all retries. Error: {llm_error_str}")
            ...
            thought, action_str, final_answer = self._parse_llm_reason_act_output(llm_response_text)
            logger.debug(f"ReAct LLM Output Parsed: Thought='{thought}', Action='{action_str}', Answer='{final_answer}'")
            await self.genie.observability.trace_event("react_agent.llm.parsed", {"thought": thought, "action_str": action_str, "final_answer": final_answer, "iteration": i+1}, "ReActAgent")

            if final_answer:
                logger.info(f"ReActAgent: Final answer received: {final_answer}")
                scratchpad.append(ReActObservation(thought=thought or "N/A", action="Answer", observation=final_answer))
                await self.genie.observability.trace_event("react_agent.run.success", {"answer": final_answer, "iterations": i+1}, "ReActAgent")
                return AgentOutput(status="success", output=final_answer, history=scratchpad)

            if not action_str:
                logger.warning("ReActAgent: LLM did not specify a valid action or final answer. Terminating.")
                ...
                await self.genie.observability.trace_event("react_agent.run.error", {"error": "NoActionOrAnswerFromLLM", "iteration": i+1}, "ReActAgent")
            ...
            try:
                ...
                logger.warning(f"ReActAgent: Failed to parse JSON params for tool '{tool_name_from_llm}': {e_json}. Params string: '{tool_params_json_str}'")
            ...
            if tool_name_from_llm not in candidate_tool_ids:
                logger.warning(f"ReActAgent: LLM chose tool '{tool_name_from_llm}' which is not in the candidate list. Informing LLM.")
            ...
            try:
                ...
                logger.error(f"ReActAgent: Error executing tool '{tool_name_from_llm}': {e_tool_exec}", exc_info=True)
