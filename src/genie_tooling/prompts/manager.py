...
class PromptManager:
    ...
    async def render_prompt(
        ...
        if not engine_id_to_use:
            await self._trace("log.error", {"message": "No prompt template engine ID specified and no default is set for render_prompt.", "error": "NoTemplateEngine"})
            logger.warning(f"No prompt template engine configured (requested: {template_engine_id}, default: {self._default_template_engine_id}). Attempting basic string.format() for render_prompt.")
            try:
    ...
    async def render_chat_prompt(
        ...
        if not engine_id_to_use:
            await self._trace("log.error", {"message": "No prompt template engine ID specified for chat and no default is set.", "error": "NoTemplateEngine"})
            logger.error(f"No prompt template engine configured for chat (requested: {template_engine_id}, default: {self._default_template_engine_id}). Cannot render chat messages.")
            return [{"role": "system", "content": f"Error: No template engine to render chat prompt. Template content was: {content_to_render[:100]}..."}]
